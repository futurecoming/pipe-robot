<template>
  <div id="role">
    ...
    <div class="content">
      ...
      <el-dialog title="添加角色" :visible.sync="addVisible" center @close="resetForm('addForm')" :close-on-click-modal="false">
      ...
      <el-dialog title="配置权限" :visible.sync="assignVisible" center @close="selectedPermitsIdx = []" :close-on-click-modal="false">
        <el-transfer
...
export default {
  ...
  methods: {
    ...
    async search() {
      ...
      let filterRoles = this.roles.filter(item => item.name === this.keywords || item.nickName === this.keywords);
